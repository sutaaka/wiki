...
#             print(page)
#             try:
#                 QUERY = 'INSERT INTO wiki.topically (parent_name, page_id, degree) VALUES (' + item + ',' + page + ',' + (len(alignArray)-i) + ')'
#                 cur.execute(QUERY)
#             except:
#                 cur.execute("INSERT INTO wiki.topically (parent_name, page_id, degree) VALUES ( '%s', %d, %d);" % (item, page,(len(alignArray)-i)))
#             conn.commit()
    ...
    try:
        for i, item in enumerate(alignArray):
            if i<1:
                ...
                QUERY += ',' + '("' + item + '","' + child + '")'
            #QUERY = 'INSERT INTO wiki.degree (parent_name, page_id, degree) VALUES (' + item + ',' + page + ',' + (len(alignArray)-i) + ')'
        cur.execute(QUERY)
        #print(QUERY)
            #cur.execute("INSERT INTO wiki.degree (parent_name, child_name, degree) VALUES ( '%s', '%s', %d);" % (item, child,(len(alignArray)-i)))
    except:
        for i, item in enumerate(alignArray):
            if i<1:
                QUERY = "INSERT INTO wiki.topic_la (parent_name, child_name) VALUES ('" +  item + "','" + child + "')"
