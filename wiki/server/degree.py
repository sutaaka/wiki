    ...
    for i, item in enumerate(alignArray):
        for j, page in enumerate(pages):
            ...
            try:
                ...
                cur.execute("INSERT INTO wiki.degree (parent_name, page_id, degree) VALUES ( '%s', %d, %d);" % (item, page,(len(alignArray)-i)))
            conn.commit()
...
def categoryIn(wordArray,indexes,child):
    ...
    try:
        for i, item in enumerate(alignArray):
            if i<1:
                QUERY = 'INSERT INTO wiki.degree (parent_name, child_name) VALUES ("' +  item + '","' + child + '")'
            else:
                QUERY += ',' + '("' + item + '","' + child + '")'
            #QUERY = 'INSERT INTO wiki.degree (parent_name, page_id, degree) VALUES (' + item + ',' + page + ',' + (len(alignArray)-i) + ')'
        cur.execute(QUERY)
