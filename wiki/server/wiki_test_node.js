...
const connection = mysql.createConnection({
  ...
  const promisedQuery2 = (word,type) =>{ return new Promise((resolve, reject) => {
    connection.query(`SELECT cl_to AS parentName, page_title AS childName, cl_from AS pageId FROM page JOIN categorylinks ON categorylinks.cl_from=page.page_id WHERE categorylinks.cl_type = "${type}" AND categorylinks.cl_to = '${word}';`, function (error, results, fields) {
        if (error) reject(error)
        resolve(results)
  })
...
const recursion = async (wordArray,index)　=>{
    ...
    wordArray[(wordArray.length - 1)].forEach(async(word,i,array)=>{
        ...
        if(results.length < 1){
            ...
            console.log('page');
            //配列[i]のサブページ取得
            const pages = await promisedQuery(word,"page");
            ...
            if(pages.length > 0){
                ...
                for(j=(wordArray.length - 1); j>-1; j--){
                    ...
                    appendFile("test.txt", wordArray[j][index[j]] + "\n");
                    //ページ情報を保存
                    //const e = await asyncMap(pages,wordArray[j][index[j]] ,degree ,insert)
                }
                //現在の要素（カテゴリ番号）を +1 して次のサブカテゴリ
            ...
            appendFile("test.txt", "\nnewWordArray\n");
            appendFile("test.txt", newWordArray + '\n');

            //index配列の最後尾に、要素番号[0]を挿入
            newIndex.push(0);
            console.log(newIndex);
            appendFile("test.txt", newIndex + '\n\n');
